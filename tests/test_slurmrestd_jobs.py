...
class TestSlurmdbJobs:
    ...
    def test_fields_match_sacct_math(self, restd, auth_headers, state_env):
        _seed_records()
        jobs = restd.get(
            f"/slurmdb/{V}/jobs/", headers=auth_headers, params={"users": "alice"}
        ).json()["jobs"]
        job = jobs[0]
    ...
    def test_account_filter(self, restd, auth_headers, state_env):
        _seed_records()
        jobs = restd.get(
            f"/slurmdb/{V}/jobs/", headers=auth_headers, params={"account": "other"}
        ).json()
        assert jobs["jobs"] == []
    ...
    def test_bad_time_spec(self, restd, auth_headers, state_env):
        _seed_records()
        response = restd.get(
            f"/slurmdb/{V}/jobs/", headers=auth_headers, params={"start_time": "bogus"}
        )
    ...
    def test_single_job_lookup(self, restd, auth_headers, state_env):
        ...
        jobs = restd.get(f"/slurmdb/{V}/job/{job_id}", headers=auth_headers).json()["jobs"]
        assert len(jobs) == 1
