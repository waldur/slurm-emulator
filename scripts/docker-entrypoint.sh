...
if [ "${SLURM_EMULATOR_NSS:-0}" = "1" ]; then
    ...
    if [ -f "$sssd_src" ]; then
        ...
        if ! install -o root -g root -m 0600 "$sssd_src" "$sssd_conf" \
            ...
            echo "WARNING: SLURM_EMULATOR_NSS=1 but sssd failed to start;" \
                 "LDAP users will not resolve (check $sssd_src)" >&2
        fi
    else
        echo "WARNING: SLURM_EMULATOR_NSS=1 but no sssd.conf found at" \
